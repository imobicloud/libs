function TabGroupManager() {
    function init(args) {
        ...
        for (var i = 0, ii = arrayTab.length; ii > i; i++) {
            var tab = arrayTab[i], UICache = new UIManager();
            ...
            tabs.push(Ti.UI.createTab({
                icon: tab.icon,
                title: tab.title,
                window: UICache.get(0)
            }));
        }
        tabgroup.setTabs(tabs);
        tabgroup.addEventListener("focus", tabGroupFocus);
        true && tabgroup.addEventListener("androidback", androidback);
    ...
    function androidback() {
        ...
        if (controller.androidback && false === controller.androidback()) return false;
        getCache(activeTab).length > 1 ? loadPrevious() : fireEvent("tabgroup:exit");
    }
